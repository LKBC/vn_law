...
def supervise(
    ...
    left_window = get_left_window(p_begin, low_tokens, WINDOW_SIZE)
    ...
    candiate_text = " ".join(low_tokens[p_begin:p_end + 1])
    ...
    list_phrase_start, list_phrase_end = get_list_phrase_contains_mention(p_begin, p_end, tokens)
    subphrases_in_list_phrase_left = get_all_phrases_in_sentence(tokens[list_phrase_start: p_begin], 10)
    ...
    legal_penalty = LegalPenaltyLabel(p_id=p_id, label=None, type=None)
    ...
    if len(NON_PENAL_SIGNALS_LEFT.intersection(phrases_in_sentence_left)) > 0:
        yield legal_penalty._replace(label=-1, type="neg:non_penalty_signals_left")

    # Rule 2: The first 2 words on the left of mention is "thi_hành án"
    if p_begin >= 2 and tokens[p_begin -1] == 'án' and tokens[p_begin - 2] == 'thi_hành':
        yield legal_penalty._replace(label=-1, type="neg:appear_left_thi_hanh_an")

    # Rule 3: The first character on the left or right is a slash (/)
    if (p_begin >=1 and tokens[p_begin -1] == '/') or (p_begin + 1 < num_tokens and tokens[p_begin + 1] == '/'):
        yield legal_penalty._replace(label=-1, type="neg:appear_slash")
    ...
    if candiate_text == 'tử_hình' and len(NON_CAPITAL_PUNISHMENT.intersection(phrases_in_sentence_left)) > 0:
        yield legal_penalty._replace(label=-1, type="neg:non_capital_punishment_signals")
    ...
    for phrase in subphrases_in_list_phrase_left:
        if regex_money_range.search(phrase.decode('utf8')) or regex_money.search(phrase.decode('utf8')):
            yield legal_penalty._replace(label=-1, type="neg:appear_other_mention_left")
            break

    # Rule 8: The sentence that contains mention is in is too short
    if num_tokens < 10:
        yield legal_penalty._replace(label=-1, type="neg:sentence_too_short")
    ...
    if len(PENAL_SIGNALS_LEFT.intersection(phrases_in_sentence_left)) > 0:
        yield legal_penalty._replace(label=1, type="pos:penalty_signals_left")
    ...
    if candiate_text == 'tù chung_thân' and ('phạt tù' in phrases_in_sentence_left):
        yield legal_penalty._replace(label=1, type="pos:life_imprisonment")
    if candiate_text == 'tử_hình' and ('phạt tù' in phrases_in_sentence_left):
        yield legal_penalty._replace(label=1, type="pos:capital_punishment")

    # Rules 3: First 2 words on the left mention is "phat tien"
    if p_begin >= 2 and tokens[p_begin - 1].lower() == "phạt" and tokens[p_begin - 2].lower() == "tiền":
        yield legal_penalty._replace(label=1, type="pos:before_phat_tien")
